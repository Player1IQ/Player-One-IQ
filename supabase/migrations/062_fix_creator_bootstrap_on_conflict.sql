-- Fix creator workspace bootstrap: team_members unique index is partial
-- (organization_id, user_id) WHERE user_id IS NOT NULL, so ON CONFLICT must
-- include the same predicate or Postgres raises:
