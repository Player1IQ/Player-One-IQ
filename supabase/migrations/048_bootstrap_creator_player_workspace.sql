-- Bootstrap solo creator/player workspaces without relying on organizations INSERT RLS
-- from clients whose JWT may not be attached to PostgREST requests.

drop function if exists public.bootstrap_creator_player_workspace(text, text);
...
begin
  ...
  set
    ...
    linked_creator_id = v_creator_id,
    updated_at = now()
